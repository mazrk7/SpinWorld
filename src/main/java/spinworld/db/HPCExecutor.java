...
public class HPCExecutor implements SimulationExecutor {
	...
	public HPCExecutor(String hpcLoginHost, String hpcLoginUser, String runScript) {
		this(hpcLoginHost, hpcLoginUser, runScript, "/opt/pbs/default/bin/qstat", 100);
	}

	public HPCExecutor(String hpcLoginHost, String hpcLoginUser, String runScript, String qstat) {
		this(hpcLoginHost, hpcLoginUser, runScript, qstat, 100);
	}

	public HPCExecutor(String hpcLoginHost, String hpcLoginUser, String runScript, String qstat, int maxQueue) {
		super();
		...
		t.schedule(new TimerTask() {
			...
			public void run() {
				...
				args.add("ssh");
				args.add(HPCExecutor.this.hpcLoginUser + "@" + HPCExecutor.this.hpcLoginHost);
				...
				for (Job j : submitted) {
					...
					if (j.status == Status.COMPLETE && original != Status.COMPLETE)
						logger.info("Simulation " + j.simId + " complete (" + j.name + ")");
				...
				logger.info("Running: " + running + ", Queued: " + queued + ", Complete: " + complete + ", Pending: "
						+ pending);
			}
		}, 5000, 10000);
	}
