...
public class PCheatChart implements TimeSeriesChart {
	...
	public void redraw(int finish) {
		...
		if (pCheats.size() == 0) {
			for (PersistentAgent a : sim.getAgents()) {
				DescriptiveStatistics pC = new DescriptiveStatistics(windowSize);
				pCheats.put(a.getName(), pC);
				for (int i = 0; i < length; i++) {
					int t = start + i + 1;
					TransientAgentState s = a.getState(t);

					if (s != null && s.getProperty("pCheat") != null) {
						double c = Double.parseDouble(s.getProperty("pCheat"));
						pC.addValue(c);
					}
				}
			}
			...
			for (PersistentAgent a : sim.getAgents()) {
				if (!pCheats.containsKey(a.getName())) {
					pCheats.put(a.getName(), new DescriptiveStatistics(windowSize));
				}

				TransientAgentState s = a.getState(finish);
				if (s != null && s.getProperty("pCheat") != null) {
					double c = Double.parseDouble(s.getProperty("pCheat"));
					pCheats.get(a.getName()).addValue(c);
				} else {
					pCheats.put(a.getName(), new DescriptiveStatistics(new double[] { 0 }));
				}
			}
		}

		List<String> agents = new ArrayList<String>(pCheats.keySet());
		Collections.sort(agents);
		for (String key : agents) {
			DescriptiveStatistics e = pCheats.get(key);
